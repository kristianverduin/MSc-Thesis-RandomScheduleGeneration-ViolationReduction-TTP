...
def createSchedules(nrTeams, n, iteration):
    ...
    for i in range(n):
        ...
        if i == 0:
            homeAwayMin.append(homeAway)
            repeatMin.append(repeat)
            robinMin.append(robin)
            total.append(value)
        else:
            lastValue = homeAwayMin[len(homeAwayMin)-1] + repeatMin[len(repeatMin)-1] + robinMin[len(robinMin)-1]
            if value <= lastValue:
                homeAwayMin.append(homeAway)
                repeatMin.append(repeat)
                robinMin.append(robin)
                ...
                homeAwayMin.append(homeAwayMin[len(homeAwayMin)-1])
                repeatMin.append(repeatMin[len(repeatMin)-1])
                robinMin.append(robinMin[len(robinMin)-1])
                total.append(lastValue)
